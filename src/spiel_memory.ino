...
void Score_mSpieler (struct s_mspieler msp[]  )
{
   ...
   for (uint8_t s = 1; s <= anzPlayer ; s++)
   { 
     ledDriver.setPixel(109+msp[s].rang,msp[s].farbe,brightness);
#ifdef DEBUG_GAME
   ...
   if ( millis() > blinkzeit + blinkdelay )
   {
      ...
      else
      {
        if (board[x][y] == SPIELER_RED)
          ledDriver.setPixel(x+m_xoffset,y+m_yoffset,RED,brightness/SPIELER_FARBE_BLINK_DIV);
        if (board[x][y] == SPIELER_GREEN)
          ledDriver.setPixel(x+m_xoffset,y+m_yoffset,GREEN,brightness/SPIELER_FARBE_BLINK_DIV);
        if (board[x][y] == SPIELER_YELLOW)
          ledDriver.setPixel(x+m_xoffset,y+m_yoffset,YELLOW,brightness/SPIELER_FARBE_BLINK_DIV);
        if (board[x][y] == SPIELER_BLUE)
          ledDriver.setPixel(x+m_xoffset,y+m_yoffset,BLUE,brightness/SPIELER_FARBE_BLINK_DIV);
        if (board[x][y] >0)
          ledDriver.setPixel(x+m_xoffset,y+m_yoffset,WHITE,brightness/FREIE_FARBE_BLINK_DIV);
        ledDriver.show();
        blinkzeit = millis();
        return true;
      }
...
{
   if (board[x][y] == SPIELER_RED)
     ledDriver.setPixel(x+m_xoffset,y+m_yoffset,RED,brightness/SPIELER_FARBE_DIV);
   if (board[x][y] == SPIELER_GREEN)
     ledDriver.setPixel(x+m_xoffset,y+m_yoffset,GREEN,brightness/SPIELER_FARBE_DIV);
   if (board[x][y] == SPIELER_YELLOW)
     ledDriver.setPixel(x+m_xoffset,y+m_yoffset,YELLOW,brightness/SPIELER_FARBE_DIV);
   if (board[x][y] == SPIELER_BLUE)
     ledDriver.setPixel(x+m_xoffset,y+m_yoffset,BLUE,brightness/SPIELER_FARBE_DIV);
   if (board[x][y] >0)
     ledDriver.setPixel(x+m_xoffset,y+m_yoffset,WHITE,brightness/FREIE_FARBE_DIV);
   ledDriver.show();
}

void fadeMPaar(uint8_t posAx, uint8_t posAy, uint8_t posBx, uint8_t posBy, uint8_t aktivfarbe)
{
    for ( uint8_t b=0 ;b<brightness-6;b=b+5)
    {
       ...
       delay(1+(brightness - b)/7);
       ledDriver.show();
    }
    for ( uint8_t b=brightness ;b>brightness/SPIELER_FARBE_DIV;b=b-5)
    {
      ledDriver.setPixel(posAx+m_xoffset,posAy+m_yoffset,aktivfarbe,b);
      ledDriver.setPixel(posBx+m_xoffset,posBy+m_yoffset,aktivfarbe,b);
      delay(25);
...
void fadeMPlayer(uint8_t prevcolor, uint8_t aktivcolor)
{
  ...
  {
    for ( uint8_t b=0 ;b<brightness/4;b++)
    {
      for(uint8_t x=0;x<FIELD_WIDTH;x++)
      {
        for(uint8_t y=0;y<FIELD_HEIGHT;y++)
        {
          ...
          {
            if ( fi == 0 && prevcolor != WHITE ) ledDriver.setPixel(x,y,prevcolor,brightness/4-b);
...
void showMBoard()
{
  ...
  ledDriver.clear();
    ...
    case 2:
      ledDriver.setPixel(111,GREEN,brightness);
    case 1:
      ledDriver.setPixel(110,RED,brightness);
    }
...
void mSpielende( struct s_mspieler msp[] )
{
   ...
   uint16_t audiolautstaerke = 33*gamesound;
         ...
         while ( PlayAudioBuffer(audiolautstaerke) || Soundaktiv() )
         {
           ...
           if ( millis() > Eblinkzeit+200 )
           {
             for (uint8_t y = 0; y < M_HEIGHT ; y++)
             {
               for (uint8_t x = 0; x < m_width_size ; x++)
               {
                 if (board[x][y] == s*-1) ledDriver.setPixel(x+m_xoffset,y+m_yoffset, msp[s].farbe,brightness*Eonoff);
               }
             } 
             if ( Eonoff ) Eonoff = 0; else Eonoff = 1;
             ledDriver.setPixel(109+msp[s].rang,msp[s].farbe,brightness*Eonoff);
             ledDriver.show();
             Eblinkzeit = millis();
           }
         }
         ...
         for (uint8_t y = 0; y < M_HEIGHT ; y++)
         {
           for (uint8_t x = 0; x < m_width_size ; x++)
           {
             if (board[x][y] == s*-1) ledDriver.setPixel(x+m_xoffset,y+m_yoffset, msp[s].farbe,brightness/SPIELER_FARBE_DIV);
           }
         } 
         ledDriver.setPixel(109+msp[s].rang,msp[s].farbe,brightness);
         ledDriver.show();
